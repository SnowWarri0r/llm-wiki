...
- [UniSwap · 一套模型同时换外观、换声音并流式输出](wiki/papers/uniswap-av.md) — 不把视频换人和变声串成两条互不相干的流水线：先用“身份被人工改乱的输入 → 恢复原片”自造音视频监督，再把冻结 LTX-2.3 分三阶段改成块流式三步学生；完整拆四路条件、flow-matching 手算、解耦 mask、三 LoRA DMD、Feature-RoPE 与事务式 cache，并严格区分论文量化到 1 分钟、项目页 hour-scale 宣称、13.6 FPS 流式但非 25 FPS 实时和代码尚未公开四个边界。
...
- [Wave Forcing · 砍一条边，换一条流水线](wiki/papers/waveforcing.md) — SJTU 流式视频算法-系统共设计（博客版，arXiv 未出）：Rolling Forcing 的同步元凶不是混合噪声窗而是「噪→净」反向注意力边；砍掉变 DAG 后每卡一个去噪阶段完美流水（Wave Parallelism），KV 单向发布、通信藏进最慢 rank 的计算窗（负载不均=overlap 预算）；copy engine 单边写+paged 直写+按实测时间切 VAE；8×H200 100 秒视频 13.56 秒出片 117.7 FPS、Drift 0.030 vs RF 0.618；含 anime.js 波前/甘特动画
- [Darwin Gödel Machine · 不证明，生下来再考试](wiki/papers/dgm.md) — agent self-evolve 奠基作（Sakana×UBC 2025）：哥德尔机「证明有益才自改」四十年落不了地，DGM 改成实测——agent 改自己代码库、基准考试、合格进档案；自指闭环（考试考的就是改代码）×开放式探索（父代选择∝sigmoid性能×1/(1+子代数)，差生也有 59.5% 概率被选）；80 代 SWE-bench 20→50、Polyglot 超 Aider；三消融各掉 10-27 分；附录 H 的 objective hacking（agent 删检测器拿满分）是给所有 agent 评测设计者的警钟
...
- [多模态预训练的经验规律 · 文字、看图和画图怎样一起长出来](wiki/papers/physics-of-multimodal-pretraining.md) — FAIR / Meta / Oxford 用真实数据、CLEVR 控制实验、内部探针与 2T 放大实验拆开 L/U/G 知识流：语言普遍帮助视觉，理解明显帮助生成，生成的零样本反哺较弱；共享 attention/norm、拆 FFN，尽早联合训练，1T 网格中 70/25/5 综合最好。完整解释 ratio、PPL、flow、vision laziness 与 MoE，并保留 1T timing sweep 的视觉 token 混杂、单 seed、5% 只是 generation-token share 等边界。
...
- [InteractiveAvatar · 记得过去，也听懂现在该做什么](wiki/papers/interactive-avatar.md) — ASR + RRM 把一轮用户语音拆成动作、回答与稳定状态；LSVM 密集保存最近 5 秒、用 SigLIP2 冗余度挑远期关键帧；prompt 改变时只刷新相邻块 KV，再以 3 步 Self-Forcing DMD 跑到 26.68 FPS。完整核算 8640 memory token、四阶段训练、2.6 秒首帧和消融，并保留双 H100 口径、Eq.8 记号缺口与未公开实现边界。
...
- [WorldTrace · 记忆还在，为什么视频世界模型找不到回家的路](wiki/papers/worldtrace.md) — NVIDIA 训练外 KV 记忆：recent 原样保留，summary 按槽位获得训练范围内的虚拟时间地址；Field 先反旋 RoPE 再分组平均保连贯，Landmark 冻结场景入口 Key 保回访。含相位抵消、P 矩阵、slot-rank、LoopBench 与极长 PAC 手算，并分清固定 GPU cache 和 Field 主实现线性增长的 CPU host state。
...
- [SPP + PAN Neck](wiki/concepts/spp-panet-neck.md) — SPP让同一位置并排看原点/5/9/13四档范围且13×13不变,四路512→concat 2048;PAN再让语义下发、位置回流
- [Spatial-wise vs Point-wise Attention](wiki/concepts/spatial-vs-pointwise-attention.md) — 原SAM每个(h,w)一个权重且全通道共享;YOLOv4改为每个(c,h,w)各自门控,用2通道2×2数值完整对照
...
- [困惑度 Perplexity](wiki/concepts/perplexity.md) — exp(平均每token负对数概率)=等效分支数,越低模型越觉自然; 逐token几何平均=已按长度归一,公平比不同长度回答; GRAPE用它给候选排序; PPL=exp(平均交叉熵)
- [分布对齐 SFT](wiki/concepts/distribution-aligned-sft.md) — 微调数据好坏是相对模型说的: 选贴合目标模型分布(给高概率/低困惑度)的回答而非绝对最强,少把模型从预训练底子拽走; on-policy>off-policy同源; 纯自产会塌缩故选外部多样(GRAPE)
