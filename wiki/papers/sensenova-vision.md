...
# SenseNova-Vision · 把整个视觉工具箱接进同一个生成接口

检测要框，OCR 要文字，分割要 mask，深度要连续场，多视图 3D 还要 point map 和相机位姿。过去的“统一视觉模型”大多统一 backbone，却保留一排 task-specific head。SenseNova-Vision 的赌注更激进：**只用 Bagel 本来就有的文本生成和图像生成通道，把异构视觉答案都改写成可解码的文本、图像或混合响应。**

## 一句话

**自然语言负责说清任务和输出协议；稀疏答案写成文本，稠密答案画成图，语义+像素答案先写颜色图例再画 mask。一个 7B MoT 模型因此覆盖结构化视觉、稠密几何、分割和多视图 3D，而不新增任务专用 head。**

## 它真正改了什么

先解释 "**head（输出头）**"：视觉大模型有个公用的"身子"（把图看懂），但检测、分割、深度、姿态每个任务后面各接一条专门的"尾巴"，把看懂的东西翻译成"框""mask""深度图"——这些尾巴就是 head，每种任务一条，结构、loss、解码全得单独设计。SenseNova-Vision 的赌注：一个统一多模态模型本来就会**写文本**和**生成图像**两件事，那就把所有任务的答案都翻译成这两种它天生会的输出（框→写成文字坐标，深度图→画成灰度图），**一条尾巴都不加**。

它不是提出了一个更强的检测 head 或分割 decoder，而是改了任务接口：

| 旧组织方式 | SenseNova-Vision |
|---|---|
| 检测 / 分割 / 深度各有专用输出头 | 共用文本与图像生成通道 |
| 任务名常由调用哪颗 head 决定 | instruction 明说任务、目标和解码规则 |
| 新任务要设计结构、loss、decoder | 先找可生成、可逆的输出表示 |
| 跨任务知识被 head 隔开 | 同一参数学习文本、像素与跨模态对应 |

这就是 [[unified-multimodal-generation]]。统一的是参数、调用入口和原生生成空间；benchmark 指标与解码协议仍然各自存在。
...
共同套路一句话:**一个像素上的量,只要不超过 3 个数,就能当颜色画出来**(因为一个像素的颜色本来就是 RGB 三个数)。深度 1 个数→灰度;法线是朝向箭头 3 个数 `(nx,ny,nz)` 各在 −1~1→线性搬到 0~255 塞进 RGB(所以法线图五颜六色其实是朝向);point map 每像素存 3D 坐标 `(X,Y,Z)`→塞进 RGB。四种任务在协议里全成了"画一张 RGB 图"。这不是为了好看，而是把稠密监督送进已有 VAE+图像生成通道。详见 [[decodable-vision-representation]]。
...
先拆几个词:**多视图**=同一场景从多个角度各拍一张(最多 10 张)反推 3D;**参考坐标系**=3D 位置得有个原点当基准,这里拿第一帧当基准,其他帧都换算过去;**内参(intrinsics)**=相机"镜头"的几个数(焦距、画面中心),配上某像素的深度就能把"屏幕第(u,v)像素、距 d 米"反算成相机眼前一个 3D 点;**外参(extrinsics)**=相机摆在哪、朝哪(旋转+平移),把相机眼前的点搬到统一坐标;**point map**=和照片一样大的图,但每像素存的是 3D 坐标 `(X,Y,Z)` 当 RGB 画出来。

每个训练样本最多随机取 10 个 view。第一个 view 是参考坐标系，其他 view 的点都变换到它的坐标系；所有 view 共同做中心化和尺度归一化。无效深度像素(如天空,定位不了)沿相机光线推到单位立方体 skybox，避免用 mask 另开一条训练逻辑。当前官方数据代码对应流程是：
...
图像先被 frozen VAE encoder 压成 clean latent `x0`，采噪声 `ε` 与时间 `τ`，走直线路径：
...
论文没有报告两项权重，不能凭这条概括式倒推出比例。当前官方仓库 `beea1f7` 的训练脚本设 `λ_text=0.25`，图像 MSE 沿用默认 `λ_image=1.0`；这是当前代码配方，不是论文表里独立消融出的结论。
