...
先把 [[pooling]] 的动作说清：max pooling 拿一个 <code>k×k</code> 窗口，在每个通道里取最大值；窗口再按 stride 移动。是否缩图由 stride 和 padding 决定，不是由“pooling”这个名字决定。
...
9×9 与 13×13 同理，所以四路都是 <code>13×13×512</code>。沿通道 concat 后得到 <code>13×13×2,048</code>：空间位置没少，每个位置却同时拿到原点、近邻、中范围和最宽 13×13 邻域四档上下文。直通支路只是原特征，常画成“1×1 视野”，但实现里没有真的做 1×1 pooling。完整滑窗手算见 [[spp-panet-neck]]。

PAN 接着做跨尺度融合：13→26→52 把深层“是什么”送给细网格，52→26→13 把浅层“在哪里”送回粗网格。YOLOv4 把原 PAN 的 shortcut 相加改成 concat：相加会立刻把两路同通道合成一个数；concat 先保留两路，再让后续卷积决定如何混合。

## Spatial-wise → Point-wise Attention · 到底从哪一级细到哪一级

设输入特征 <code>F ∈ R^(C×H×W)</code>。其中 <code>C</code> 是通道数，<code>H,W</code> 是空间尺寸，<code>F[c,h,w]</code> 是一个具体通道、具体位置上的标量。
...
代价也更明确：空间门只有 <code>H×W</code> 个数，point-wise 门则有 <code>C×H×W</code> 个数，表达更细但生成门控更贵。论文 Table 5 中 SPP/PAN 基线 42.4 AP，加 SAM 后 42.7 AP；这不是 spatial-wise 与 point-wise 的严格一对一消融，不能把 +0.3 全归因于修改粒度。

还有一个实现版本陷阱：论文 Fig. 5 画了 modified SAM，官方 <code>sam_layer.c</code> 也支持逐元素门控；但当前官方仓库 <code>master@59596d7</code> 的标准 <code>yolov4.cfg</code> 没有 <code>[sam]</code>，SAM 出现在实验 cfg 中。论文方法清单与某个默认 cfg 不能混写成一件事。
