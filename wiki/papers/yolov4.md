...
**保留 YOLOv3 检测头，用 CSPDarknet53 换掉主干，用 SPP/PAN 重搭特征融合层，再把训练期技巧和部署期组件分开筛选。**

## 它要解决的痛点

- **论文技巧太多，不能看见 SOTA 就全装**：有些只对分类有效，有些组合后反而掉点，必须在大数据集和完整检测器上实测。
- **理论运算少，不等于 GPU 真跑得快**：分组卷积、碎片化算子可能降低 BFLOPs，却喂不满 GPU；部署时最终要看实测吞吐。
- **高精度检测器太吃训练资源**：大 batch、同步 BN 和多卡训练把复现门槛抬高。YOLOv4 明确把单张 1080 Ti / 2080 Ti 当约束。
...
4. **注意力粒度**：[[spatial-vs-pointwise-attention]] 把所有通道共享的空间门，改成每个通道、每个位置各自门控。
5. **训练增强**：[[mosaic-augmentation]] 一张训练画布拼四张图，顺手混入四种上下文；另加 SAT、DropBlock、标签平滑等训练技巧。
6. **框损失**：[[complete-iou-loss]] 同时处罚重叠不足、中心偏移和长宽比不合。
...
2. Related Work：把检测器拆成主干、特征融合层、检测头，再盘点训练期和推理期候选技巧。
3. Methodology：选 CSPDarknet53、SPP、PAN，修改 CmBN、SAM 和 PAN，引入 Mosaic 与 SAT。
4. Experiments：先做 ImageNet 分类组件实验，再做 COCO 检测消融、骨干与小批次对照。
5. Results：在 Maxwell、Pascal、Volta 三代 GPU 上分别比较速度与精度。
6. Conclusions：强调基于 anchor 的一阶段检测仍有竞争力，而且整套模型能在普通单卡上训练。

## 先把检测器拆成三段

YOLOv4 没换掉 YOLOv3 基于 anchor 的三尺度检测头。真正重搭的是前两段：
...
CSP 不是“随便把通道砍半”。它想解决的是：别让所有通道都重复跑同一串残差块。一个 stage——也就是一组保持同样分辨率的网络层——先分流：一路进残差块反复加工，另一路走短路，末尾再拼回来，用 1×1 卷积融合。
...
✓ 两路都进入下一 stage，只有一路反复经过残差块。真实网络会在整张特征图上做同样的分流与拼接，这里只是拿一个空间位置的四个通道手算。

论文的硬件表也提醒了一件反直觉的事：512 输入时，CSPDarknet53 是 52 BFLOPs、66 FPS；CSPResNeXt50 只有 31 BFLOPs，却是 62 FPS。理论运算少了，不代表 GPU 上一定更快。

## SPP + PAN · 池化不是一定缩图，而是窗口汇总

先把 [[pooling]] 说清楚：最大池化拿一个 <code>k×k</code> 窗口，在每个通道里取最大值。窗口再按步长移动。特征图会不会缩小，由步长和补边共同决定，不能看到“pooling”就默认它会下采样。

YOLOv4 在最深的 <code>13×13×512</code> 特征上保留原特征直通，再让 5×5、9×9、13×13 三个 max-pool 分支从同一输入并排取值。三路都是 <code>stride=1</code>，padding 分别为 2、4、6。输出高度公式是：

~~~text
H_out = floor((H + 2p − k) / s) + 1
~~~

<code>H</code> 是输入高度，<code>k</code> 是池化核，<code>p</code> 是单侧 padding，<code>s</code> 是 stride。拿 5×5 分支代入：

~~~text
H_out = floor((13 + 2×2 − 5) / 1) + 1 = 13
~~~

9×9 与 13×13 同理，所以四路都是 <code>13×13×512</code>。沿通道拼接后得到 <code>13×13×2,048</code>：空间位置一个没少，但每个位置同时拿到了原点、近邻、中程和最宽 13×13 邻域四档信息。直通支路只是原特征，常被画成“1×1 视野”，实现里并没有真的做 1×1 池化。完整滑窗手算见 [[spp-panet-neck]]。

PAN 接着连通三个尺度：13→26→52 把深层的类别语义送给细网格，52→26→13 再把浅层的精确位置送回粗网格。YOLOv4 把原 PAN 的 shortcut 相加改成 concat 拼接：相加会立刻把两路同通道压成一个数；拼接先完整保留两路，再让后续卷积决定怎么混。

## Spatial-wise → Point-wise Attention · 从“每个位置一扇门”到“每个元素一扇门”

先看直觉：原版给每个空间位置一个权重，同一位置的所有通道共用；改版给每个通道、每个位置单独一个权重。这样，同一个位置上的“轮廓”通道可以保留，“纹理”通道可以压低。

再看公式。设输入特征 <code>F ∈ R^(C×H×W)</code>，<code>C</code> 是通道数，<code>H,W</code> 是空间尺寸，<code>F[c,h,w]</code> 是某个通道、某个位置上的一个数。<code>A</code> 表示 0～1 的门控权重，<code>Y</code> 表示加权后的输出。

原 spatial-wise SAM 先沿通道做 average/max pooling，再生成一张 <code>1×H×W</code> 空间权重图：
...
<code>avg_c/max_c</code> 表示在同一位置沿 C 个通道求平均/最大；<code>conv</code> 生成空间分数；<code>sigmoid</code> 把它压到 0~1。关键限制是：同一位置所有通道共享 <code>A_sp[h,w]</code> 这一个门。

YOLOv4 的 point-wise SAM 改为生成同形状的 <code>C×H×W</code> 门控：
...
这里的 point 是张量里的单个元素 <code>(c,h,w)</code>，不是只有二维坐标 <code>(h,w)</code>。例如同一左上角有两个通道值 <code>2</code> 和 <code>8</code>：spatial-wise 若给该位置 0.25，两者只能同时变成 0.5 和 2；point-wise 可以分别给 0.25 和 0.75，得到 0.5 和 6。于是模型能在同一位置保留某类特征、压低另一类特征。完整 2×2 双通道手算见 [[spatial-vs-pointwise-attention]]。

代价也很直接：空间门只有 <code>H×W</code> 个数，point-wise 门则有 <code>C×H×W</code> 个数。后者分得更细，但生成门控也更贵。论文表 5 中，SPP/PAN 基线是 42.4 AP，加 SAM 后是 42.7 AP。这只能说明“这套组合加 SAM 后高了 0.3 AP”；论文没有把两种 SAM 单独摆在同一组实验里比较。

还有一个版本差异：论文 Fig. 5 画了 modified SAM，官方 <code>sam_layer.c</code> 也支持逐元素门控；但官方仓库 <code>master@59596d7</code> 的标准 <code>yolov4.cfg</code> 没有 <code>[sam]</code>，SAM 只出现在实验 cfg 里。因此，照论文画结构图和照默认 cfg 复现，不一定得到同一张网络图。
...
- <code>w,h,w_gt,h_gt</code>：预测框和真值框宽高；<code>atan(w/h)</code> 把长宽比转成角度，两个角度相差越大，<code>v</code> 越大。
...
mini-batch 实验只能得出一个有限结论：带 BoF/BoS 的 CSPDarknet53 在 mini-batch 4 与 8 时是 41.6 vs 41.7 AP，差 0.1；不带这些策略的另一套 CSPResNeXt50 是 37.1 vs 38.4，差 1.3。这说明该配方在这组实验里对小 mini-batch 更稳，不能外推成“batch size 永远无所谓”。
