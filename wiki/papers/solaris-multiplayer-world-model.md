...
相邻两行有 `L_s−1` 帧是同一批帧，但它们是两次独立的前向，各留各的激活，谁也替不了谁，于是总量是 `O(L_t·L_s)`。
...
朴素滑窗：20 行 × 每行 6 帧 = 120 份
两遍法  ：一条序列 = 20 干净 + 20 带噪 = 40 份   → 省 3 倍

把窗口放宽到 L_s = 12：
朴素滑窗：20 × 12 = 240 份
两遍法  ：仍是 40 份（式子里压根没有 L_s）      → 省 6 倍
```

真正买到的东西不是那 3 倍，而是**显存不再跟 student 的窗口长度挂钩**，只按生成了多少帧线性涨。这就是论文说的“把自回归 rollout 与反向传播解耦”。
...
两者都 `stop_gradient`。它们照样参与前向、照样影响损失，只是梯度不再往回穿第一遍那条又长又串行的生成链，于是那条链的激活可以当场丢掉。
