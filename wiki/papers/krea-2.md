...
中训为什么还要单独检查具体实体？假设一个“运动员”簇有 10,000 张图片，其中某位知名球员只有 10 张，占 `0.1%`。如果为了节省训练算力，只从这个簇随机保留 100 张，那么留下这位球员图片的期望数量只有 `100×0.1%=0.1` 张；一张都没抽中的概率约为 `(1−0.001)^100≈90.5%`。原始数据虽然认识这个人，抽样后的训练集却很可能把他完全删掉。
