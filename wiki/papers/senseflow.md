...
§4 的 IDA 修的是“fake 追不上学生”。这节是另一个病：**监督点太少**。ISG 全称 intra-segment guidance，段内引导。

### anchor 是什么，为什么只有四个

蒸出来的是**四步**生成器：推理时只在四个时刻各前向一次。这四个时刻叫 anchor，训练时 DMD 损失也只加在这四个点上——**整条轨迹上除了这四个点，生成器收不到任何信号**（论文原话：generator receives no signal over the rest of the trajectory）。

先把时间轴方向钉死，否则后面全反：**`τ=1.0` 是纯噪声，`τ=0` 是干净图**，去噪从 1 往 0 走，所以 anchor 表要从右往左读。
...
直觉上 `ξ(t)` 该单调：噪声越大越难猜。**实测不是**，曲线会局部上下抖，`t∈[.8,1.0]` 尤其明显——而 `τ₃=1.0` 恰好就是一个 anchor。

因果链摊开：① 某 anchor 落在 `ξ` 的局部尖峰上 → 老师自己在这个时刻就答得偏；② 学生在该 anchor 上对齐老师 → 学到的方向跟着偏；③ 四步一共只有四个点，这个 anchor 要独自代表左右一整段 → 局部坏点被放大成整段错误。论文的说法是：把邻近时刻当成同等有信息量（treating nearby timesteps as equally informative），会把训练锚死在次优点上。
...
走一步用最朴素的欧拉法，先讲清否则数字对不上：

\[
x_{\text{new}}=x_{\text{old}}+(t_{\text{new}}-t_{\text{old}})\cdot v.
\]

**符号陷阱：去噪时 `Δt` 是负的。**从 `.75` 走到 `.60`，`Δt=−.15`；速度 `v` 为负时 `Δt·v` 反而为正，`x` 会变大。下面每步都撞上这个负负得正。

取 `τ_i=.75、t_mid=.60、τ_{i-1}=.50、x_{τ_i}=.20`：

```text
路线 A 前半段 · 老师走 · 不回传
  Δt = .60 − .75 = −.15,  v_real = −2.0
  x_mid = .20 + (−.15)×(−2.0) = .20 + .30 = .50

路线 A 后半段 · 学生走 · 不回传
  Δt = .50 − .60 = −.10,  v_G = −1.5
  x_tar = .50 + (−.10)×(−1.5) = .50 + .15 = .65

路线 B · 学生一步跨完 · 回传
  Δt = .50 − .75 = −.25,  v_G = −1.0
  x_direct = .20 + (−.25)×(−1.0) = .20 + .25 = .45

L = (x_direct − x_tar)² = (.45 − .65)² = .04
```

自检：两条路都从 `t=.75` 到 `t=.50`，各自 `Δt` 加起来也对得上（`−.15 + −.10 = −.25`）。差别只在 A 中途换了一次速度、B 一路一个速度——**这个差就是段内信息**。

### 三个容易滑过去的“为什么”

**为什么每次重抽 `t_mid`？**一个 `t_mid` 只注入段内某一处的老师行为；随机换，多轮之后这个 anchor 就吸收了整段。注意这条绕路**只在训练时存在**，推理仍是四步，没有变慢。

**为什么必须 stopgrad？**目标 `x_tar` 里也含学生，不冻的话梯度会同时推两边：不用把 B 学对，只要 A 和 B 一起塌到同一个平凡值，损失照样降到 0。stopgrad 把 A 钉成常数，B 才只能真往它靠。

**为什么目标支路留学生走后半段，不让老师走完整段？**论文只说这样能把老师在段内的行为聚合进来（aggregates the teacher's behavior within each segment），**没有正面论证为什么不让老师独走**。可观察到的两点：老师走完整段要多跑好几个小步、成本更高；且那样目标纯是老师的轨迹终点，跟“学生一步该落在哪”是两回事。这属于合理推测，不是论文的论证。
...
代码还在 SD 3.5 的两个端点各留 50 个离散 index 再采 `t_mid`——避免抽到贴着端点、让某半段长度接近 0 而白走，并用 CFG=5 的老师走前半段。复现时应以对应版本代码为准，不能只照正文公式。
