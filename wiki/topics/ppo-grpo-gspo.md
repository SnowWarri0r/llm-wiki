...
```text
[1.10×.6, 1.05×.6, .90×.6, min(1.25×.6, 1.20×.6)]
= [.66, .63, .54, .72]
```
...
```text
s_i = exp( (1/4) × [log 1.10 + log 1.05 + log .90 + log 1.25] )
    = (1.10 × 1.05 × .90 × 1.25)^(1/4)
    ≈ 1.0677
```
...
```text
每条回答的目标 = (1/|y_i|) × Σ_t token_loss(i,t)
```
...
```text
每条回答的目标 = (1/M) × Σ_t token_loss(i,t)
```
...
s_i(θ) = [ π_θ(y_i|x) / π_old(y_i|x) ]^(1/|y_i|)
       = exp( (1/|y_i|) × Σ_t log[π_θ(y_i,t|x,y_i,<t) / π_old(...)] )
```
...
```text
L_GSPO = E_x,{y_i} [ (1/G) × Σ_i min(s_i A_i, clip(s_i,1−ε,1+ε) A_i) ]
```
