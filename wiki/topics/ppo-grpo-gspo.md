...
以数学题 \(17 \times 6 = ?\) 为例，一轮在线 RL 是：

1. 用当前策略的快照 \(\pi_{\mathrm{old}}\) 对同一道题采样若干回答。
2. 验证器或 reward model 给每条完整回答打分。
3. 把 reward 变成 advantage：正数表示“应提高这条回答的概率”，负数表示“应降低”。
4. 用 \(\pi_{\mathrm{old}}\) 采到的 token 训练新策略 \(\pi_\theta\)。因为数据来自旧策略，要用新旧概率之比校正；clip 再限制单轮更新别走太远。
5. 更新后重新采样，进入下一轮。

\(\pi\) 读作 policy（策略），在 LLM 里就是“给定题目和已生成前缀，下一个 token 的概率分布”。下标 `old` 表示采样数据时冻结的旧快照；\(\theta\) 表示正在训练的模型参数。
...
\(s_t = (x,y_<t)\) 是写到第 `t` 个 token 前的状态；\(V_\phi (s_t)\) 是 critic 对“从这里继续写，预计最终能拿多少 reward”的预测，\(\phi\) 是 critic 的参数。实际工程通常用 [[gae]] 把多步 TD 误差平滑起来，而不是只做一次相减。
...
| token | \(\pi_{\mathrm{old}}\) | \(\pi_\theta\) | \(\rho_t =\pi_\theta / \pi_{\mathrm{old}}\) |
|---|---:|---:|---:|
| 1 | 0.50 | 0.55 | 1.10 |
| 2 | 0.40 | 0.42 | 1.05 |
| 3 | 0.60 | 0.54 | 0.90 |
...
它就是四个 token 比率的几何平均。`1.0677` 没越过 `[0.8, 1.2]`，所以整条回答的目标贡献约为 \(1.0677 \times 1 =1.0677\)。
...
题 A 的正确回答 advantage 是 `1`；题 B 的正确回答是 \((1- .25)/.433 \approx 1.732\)。因为题 B 的组内标准差更小，同样一个正确样本会收到更大的更新权重。极易或极难的问题常出现 reward 几乎全相同，标准差很小；除以它会把少量差异放大。
...
普通 PPO 对比率使用对称 \([1-\epsilon ,1+\epsilon ]\)。DAPO 拆成 `[1−ε_low, 1+ε_high]`，论文实验取 \(\epsilon_{\mathrm{low}}=0.2\)、\(\epsilon_{\mathrm{high}}=0.28\)。
