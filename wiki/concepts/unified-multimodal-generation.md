...
## 一句话
不再给检测、分割、深度和 3D 各接一颗专用 head，而是先把答案翻译成模型本来就会生成的**文本、图像或文本+图像**，再用自然语言说明这次要走哪条输出通道。
...
三条请求共享同一套参数。A 用 next-token 交叉熵训练；B、C 的图像部分进 VAE latent，用 flow matching 目标训练。最终仍要按约定把 A 解回像素框、把 B 解回深度、把 C 的颜色解回类别 mask。

## 它统一了什么，没统一什么

| 统一了 | 没有自动统一 |
|---|---|
| 模型参数与调用入口 | 各 benchmark 的指标 |
