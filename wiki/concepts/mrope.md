...
## Qwen3-VL 的 Interleaved-MRoPE · 每个轴都需要快慢频率

原版 M-RoPE 把旋转维度连续切成时间、行、列三段。频率又沿维度从慢到快排列，于是三个轴各自只覆盖一段频带，分配并不均衡。问题不只是“时间轴少了某类频率”，而是三个空间—时间轴都没有同时拿到粗粒度和细粒度的位置刻度。

Qwen3-VL 的 **Interleaved-MRoPE** 改成**交错分配**：t/h/w 三个轴都横跨从慢到快的全频段，每个轴同时获得适合长距离关系与局部差异的频率。它不增加维度或 attention 次数，只改变维度分配。报告认为这有利于长视频，但没有给这一项单独开关的消融，不能从报告中拆出独立增益。
