...
name: score-function
type: concept
sources: [flow-matching, ode-sde, dmd, dmd2, data-forcing-distillation, senseflow, drifting-models, rcm, asyncpatch-diffusion]
updated: 2026-08-14
---
...
| 原始清晰照片 | 真实训练样本 \(x_{0}\) |
| 破坏程度 | 噪声时刻 `t` |
| 店里生成的雪花 | 高斯噪声 \(\epsilon\) |
| 盖上雪花的照片 | 带噪样本 \(x_{t}\) |
| 学徒 | 去噪 / score 网络 |
| 猜出的雪花 | \(\hat{\epsilon}\theta (x_{t},t)\) |
| 标准答案 | 当初亲手加进去的 \(\epsilon\) |
...
如果 `x` 是一个 \(4\times 64\times 64\) latent，score 也是 \(4\times 64\times 64\)。可以把它理解成给这 16,384 个数各写一条“小修多少”的批注。
...
- `p(x)`：目标数据在 `x` 附近的概率密度；
- `log`：对概率密度取对数；
- \(\nabla_{x}\)：逐个改变 `x` 的每个元素，找出 `log p(x)` 上升最快的方向；
- \(s_p(x)\)：最终得到的方向张量。

## Real-score 网络到底怎么训练

从真实数据抽一张干净图 \(x_{0}\)，再采噪声时刻 `t` 和高斯噪声 \(\epsilon\)：
...
- \(\alpha_{t}\)：这一时刻还保留多少原图；
- \(\sigma_{t}\)：这一时刻加入多少噪声；
- \(\epsilon\)：程序自己采样的噪声，形状与图片相同；
- \(x_{t}\)：交给网络的带噪图片。

网络看见 \(x_{t}\) 和 `t`，预测当初加入的噪声：
...
- \(\psi\)：real-score 网络的参数；
- \(\epsilon \psi (x_{t},t)\)：网络猜出的噪声；
- \(\epsilon\)：程序真正加进去的噪声；
- \(\| a-b\| _{2}^{2}\)：对应元素作差、平方，再求和或平均；
...
负号的原因也很直接：\(\epsilon\) 是刚才“加坏”的方向；要回到更像干净数据的区域，就要朝相反方向改。再除以 \(\sigma_{t}\)，是把不同破坏程度换到可比较的尺度。
...
第二步，网络看见的只有 \(x_{t}=0.66\) 和噪声时刻。假设刚开始预测 \(\hat{\epsilon}=-0.3\)：
...
同一件事也可以让网络直接预测干净值 \(\mu (x_{t},t)\)。两种写法可以互换：
...
结果和 \(-\hat{\epsilon}/\sigma\) 完全一致。DMD 论文主要使用这种“预测干净图，再换算 score”的记法。
...
|---|---|---|
| \(\mu_{\mathrm{real}}\) | 真实训练图片 | DMD 开始前已经训练好；DMD 中冻结 |
| \(\mu_{\mathrm{fake}}\) | 学生刚生成的图片 | DMD 训练期间持续更新 |

DMD 把同一个学生带噪样本 \(x_{t}\) 同时交给两套网络：
...
扩散模型并不需要先算出完整的 `p(x)`；上面的加噪训练直接学到了 \(\nabla_{x}\log p_{t}(x)\) 这根局部方向。
