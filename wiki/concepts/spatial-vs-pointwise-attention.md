...
原 spatial-wise attention 对每个空间位置只给一个权重，所有通道共享；YOLOv4 所说的 point-wise attention 给 <code class="m">C\times H\times W</code> 中每个元素一个权重，所以同一位置的不同通道可以一升一降。

## 先把“point”说准确

设输入特征为 <code class="m">F \in R^(C\times H\times W)</code>：
...
原 SAM 先沿通道做 average pooling 和 max pooling，得到两张 <code class="m">1\times H\times W</code> 地图，再拼接、卷积、sigmoid：
...
- <code class="m">A_{\mathrm{sp}}[h,w]</code>：位置 <code>(h,w)</code> 唯一的空间权重。
...
- spatial-wise 门只有 <code class="m">H\times W</code> 个值，便宜、解释直接，强调“哪里”。
- point-wise 门有 <code class="m">C\times H\times W</code> 个值，表达更细，可以同时编码“哪里”和“哪类特征”，但门控张量与生成它的 1×1 卷积都更贵。
...
论文 Fig. 5 明确画了 modified SAM。官方 Darknet 的 <code>sam_layer.c</code> 也逐元素执行 <code class="m">output[i] =input[i] \times from[i]</code>；实验配置 <code>yolov4-sam-mish-csp-reorg-bfm.cfg</code> 会先用 1×1 卷积和 logistic 生成同形状门控。
