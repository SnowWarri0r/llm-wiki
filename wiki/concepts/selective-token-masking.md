...
```
✓ 自检：三行用的是**同一个 20B 模型**，差别只在"哪几格是 🔒、哪几格是 ⚙"。\(image\to layers\) 把合成图设 clean、其余 noisy，模型就在"看着这张平图"的条件下把图层 token 去噪出来——等于学会了"看平图、拆图层"。这比"每个任务训一个网络"省得多，且三任务互相之间还能借力（[[mrt]] 实测多任务联训几乎不掉点）。
