...
## 温度 T · 尖锐 ↔ 平滑的旋钮
把 logits 先除以温度 `T` 再 softmax:\(p_{i} =\operatorname{softmax}(z_{i} / T)\)。
...
```
(还有个工程细节:实现时先减去最大值 \(\exp (z_{i} -\max z)\) 防 `exp` 溢出,结果不变。)

## 落点
- **[[self-attention]]**:\(\operatorname{softmax}(Q\cdot K^{T}/\sqrt{d})\) 把注意力分数变成"分配给每个位置的权重",和为 1。那个 \(\sqrt{d}\) 就是防分数过大、softmax 太尖(同 [[qk-rmsnorm]] 一类稳定动机)。
