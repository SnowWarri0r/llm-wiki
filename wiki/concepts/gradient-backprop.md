...
type: concept
sources: [drifting-models, drift-ar]
updated: 2026-07-24
---
...
## 数字例子 · 手算一步
一个最小网络:\(L = (w\cdot x -y)^{2}\),给定 `x=2, y=5`,当前 `w=3`,学习率 `lr=0.1`。
...
```
**验证变好了**:新 \(a=2.6\cdot 2=5.2\),`err=0.2`,`L=0.04` —— 从 1 掉到 0.04,loss 真降了。这一步就是 SGD 的全部:前向出 loss、反向出梯度、沿负梯度挪一点。
