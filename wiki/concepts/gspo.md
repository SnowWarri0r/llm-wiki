...
然后每个 \(\rho_i,t\) 各自 clip。于是同一条好回答里，某些 token 可能按 `1.2` 奖励，另一些按 `0.8` 或未截断值更新。若训练和推理引擎精度不同，或 MoE 路由让局部 log-prob 更敏感，个别 token 的比率波动会改变 clip 决策。
...
- \(\pi (y_i|x)\)：整条回答的似然，即各 token 条件概率连乘。
- \(s_i\)：这条回答唯一的序列级重要性比率。
- \(|y_i|\)：回答 token 数。
- \(1/|y_i|\)：长度归一；把连乘后的总比率拉回“平均每 token”尺度。
- \(\exp (mean(\log \rho ))\)：与上式完全等价，也是 token 比率的几何平均，数值计算时更稳定。
