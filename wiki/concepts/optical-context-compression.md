...
**SAM-ViT（前半，~80M，窗口注意力）**：来自 Segment Anything，预训练目标是"把图里每个东西精确切出来"，所以它**擅长密集、局部、精确定位的视觉细节**——刚好是认字需要的（每个字的形状、位置）。它用**窗口注意力**（只看局部邻域，不做全局），代价低，所以能在**海量高分辨率 token**（4096 个）上跑而不爆显存。它负责"看清"。
