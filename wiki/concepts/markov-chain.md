...
普通链下一步可能是个确定位置。**高斯**马尔可夫链是说：每步的转移核是个**正态分布（钟形）**——给定 \(x_{\mathrm{now}}\)，下一步 \(x_{\mathrm{next}}\) 从一团**钟形雾**里抽：
...
## 代码出处
- 概念性：任何 DDPM 实现的 \(p_{\mathrm{sample}}\) 循环——每步 \(x =\mu_\theta (x,t) +\sigma_t \cdot z\)（z~N(0,1)），就是一次高斯转移
- [[ode-sde]] 的 Euler–Maruyama 一步 \(x +=f\cdot dt +\sqrt{d}t\cdot g\cdot z\) 是它的连续时间版
