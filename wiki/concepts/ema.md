...
- **[[senseflow]] 的 IDA**：公式同样是 \(\phi ←\lambda \phi +(1-\lambda )\theta\)，但不是模型追自己的历史。\(\phi\) 属于 fake 网络，\(\theta\) 属于刚更新的生成器；这次插值负责把学生在稀疏 anchor 上的新变化软传给 fake 网络，fake 随后仍用自己的稠密时间步去噪损失继续训练。
...
- **[[drift-ar]] 的推测解码阈值**：目标模型每次验证都会产生新的注意力熵，先用 `0.9·旧平均+0.1·新熵` 平滑，再乘 \(\gamma =0.8\) 作为动态停止线，避免某一块图像的瞬时熵把策略带偏。
