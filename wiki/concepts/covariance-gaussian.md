...
## 一句话
方差说一个维度"散多开";**协方差矩阵 Σ** 说多个维度"各自散多开 + 两两一起动不动"——它描述一团数据的**椭圆形状**。妙处:**Σ 的特征向量 = 这个椭圆的轴 = PCA 主成分**(又是 [[eigenvector]] 那条不转的轴),多元高斯就是这个椭圆形状的钟形云 \(N(\mu ,\Sigma )\)。

## 直觉 · 从方差到协方差

- **方差**(1D):数据离均值平均有多远的平方,`Var(x)=平均((x−μ)²)`。一个数,说"散多开"。
- **协方差**(两维一起):`Cov(x,y)=平均((x−μₓ)(y−μᵧ))`,说**x 和 y 是不是一起动**:
  - `>0` 同涨同跌(x 大时 y 也大);`<0` 一涨一跌;\(\approx 0\) 没关系。
...
## 多元高斯 N(μ, Σ)
一维高斯是钟形曲线 \(N(\mu ,\sigma^{2})\);多维就是 \(N(\mu ,\Sigma )\)——\(\mu\) 定中心、\(\Sigma\) 定**椭圆形状**(圆形 = Σ 是 σ²I,各向同性;歪椭圆 = Σ 有非对角项)。扩散模型、VAE 的隐变量、概率建模全在用它。两个**同协方差**高斯的 KL 还会塌成均值差的平方(见 [[closed-form-kl]])。
