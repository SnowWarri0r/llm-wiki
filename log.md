...
## [2026-08-12] revise | dyaplex §05 公式讲解补洞
- 用户指出 §05 公式讲解有问题。三处坐实：RoPE(x,p) 被当已知函数甩出、全页无定义；"点积只看时间差"是断言无推导；波浪号、K^ℓ 来路、V 不旋转全没交代。
- 补法：60 秒 RoPE 底子（每两维=平面箭头,按位置×每步角度旋转）→ 角度相减恒等式 ⟨R(aφ)q,R(bφ)k⟩=⟨q,R((b−a)φ)k⟩ → 90° 手算（同格 1/差一格 0/差两格 −1,python 复核）→ 主公式+六格符号表（RoPE 定义、波浪号、K 由冻结隐藏状态经新投影、V 不旋转）。
- lint 抓到真问题：旋转角 θ 与 §06 损失里的模型参数 θ 同页撞名,角度改名 φ。0 ERROR 0 WARN、KaTeX 0 失败。
- 备注：整页重构的内容被并行会话的宽 git add 卷进了 6ad7a23（Wan-Animate 2）提交,内容无损、归属错位,记录在案。

## [2026-08-12] revise | dyaplex §05 补代入展开步
...
## [2026-08-13] revise | continuous-time-consistency 补链式法则推导
- 用户问 rcm §03 的 df/dt=∂f/∂x·dx/dt+∂f/∂t=0 为啥成立。概念页有数字例但链式法则本身无推导、"=0"只有断言。
- 补三层：①一阶泰勒推出全导数（两扇门直觉：位置动一扇、时间标签动一扇，贡献相加）；②"=0"非恒等式，是离散条件 f(x_{t−Δt},t−Δt)=f(x_t,t) 相减除 Δt 取极限强加出来的要求；③数字例补两扇门读法（位置门 +2、时间门 −2 对消，答案钉在 1）。

## [2026-08-13] fix | continuous-time-consistency 行内公式载体
- 上一笔补推导时行内公式用了裸括号 (…)，自动渲染页的行内数学约定是 \(…\)，原样漏到页面（用户截图指出）。我加的 5 处已转 \(…\)；旧文 7 处恰被并行会话的 Phase B 迁移转掉。浏览器核过全部渲染。

## [2026-08-13] revise | rcm §03 补 sCM 损失的来路
- 用户问 L_sCM 为啥长这样。原文只说"可做 MSE 的归一化切线目标"，三处靠断言。
- 补齐（对照 arXiv 2510.08431 Eq 4 核实）：①为什么不直接最小化 ‖df/dt‖²（穿 Jacobian=二阶导）；②连续时间 CM 经典结果——离散损失 Δt→0 的梯度=student 输出×冻结 target 切线，二阶导被 θ⁻ 绕开；③MSE 是装回该梯度的代理目标+不动点刻画（g=0 ⟺ 修正项消失 ⟺ df/dt=0 达成）；④为什么在 F 空间（TrigFlow 下 F=原生速度头，f 空间要除 sin t 病态）；⑤归一化=抑强扶弱（g=.3→1.579、g=10→.0999、上限 g=√c≈.316 处 1.58，python 复核），sCM FP16 真炸过。

## [2026-08-13] revise | rcm §06 拆 DMD 目标的两层壳+分母
- 用户问：这个 DMD 和熟悉的不一样、为什么除以 x0−f_teacher。
- 补齐：①壳一=score 差换 x0 空间（Tweedie 系数只含 t 被归一化吸收）；②壳二=伪回归是梯度载体（y=sg[x0−g] ⟹ ∂L/∂x0=2g，与 data-forcing 页 MIX 恒等式同款，加了 cross-link）；③分母=DMD1 提出、DMD2 沿用的稳定器——分子量级随噪声剧变，除以 teacher 偏差的自然尺度变成无量纲相对修正；与 sCM 项 ‖g‖²+c 同一门抑强扶弱手艺，只是拿谁做尺不同。

## [2026-08-13] revise | rcm §03 推开「经典结果」黑箱
- 用户追问：离散损失 Δt→0 梯度=student 输出×冻结切线，为什么。原文拿"经典结果"四字顶包。
- 补三步推导：①梯度只穿含 θ 的前项；②对 target 泰勒展开，θ⁻=当前参数值 ⟹ 同函数同点值差恒为零，残差坍缩成 Δt×冻结切线；③除 Δt 取极限。二阶导消失的机制点明（Jacobian 全在不吃梯度的 θ⁻ 分支）。
- 标量验证：f_θ=x−θt、轨迹 1+2t、θ=1.5：离散 ∇L/Δt=−.5 与极限式 2×.5×(−.5)=−.5 严丝合缝（python 复核）；顺带显现不动点（切线=2−θ，θ→2 时归零停训）。

## [2026-08-14] revise | rcm §03 补三个被当已知的前置
- 用户明说没有这类数学直觉：二阶导是不是平方、Jacobian 为啥贵、标量验证在干啥、stopgrad 通路是不是"变成常数 1"。
- 补齐：①二阶导=斜率的斜率（对导数再求导，非平方）；‖df/dt‖² 的罪在括号里已含一次求导→反传=对求导过程再求导=图套图，Jacobian 万亿格再挂反传线即爆；②sg/detach 语义=抄数值剪来历，冻结项是叶子常数、对 θ 导数是 0（不是 1）；"数值为零梯度仍流出"=同数相减为零但前项挂着活反传线；③验证=对账——等式两边各自独立算（账本一离散硬求导/账本二极限公式直代），两路同得 −.5 等式才可信；calc 标签改为账本一/账本二。

## [2026-08-14] revise | rcm §03 配三张图 + 修顺序 bug
- 用户：这么一大段文字就不能配点图吗。三个核心点各配一张 SVG（页面调色板）：Fig 03 图套图（普通损失求一次导 vs 损失含 df/dt 时反传要穿进求导图内部）；Fig 04 sg 的活线与死线（同值 1.25 相减为零，左线活右线剪，梯度流出 vs 0）；Fig 05 两本账对账（离散/极限两列同收敛到 −.5，赠品动力学）。旧 Fig 03–06 顺延为 06–09。
- 顺手修盲改造成的顺序 bug：标量验证块此前插在「下面的 MSE」与 L_sCM 公式中间；现推导→Fig B→验证框→Fig C→账本 calc→「对账通过」桥接→公式，行文连续。
- lint 0/0、KaTeX 0 失败、浏览器逐图核过（文字无压线）。
...
## [2026-08-18] ingest | UniSwap · 一套模型同时换外观、换声音并流式输出
- 按“任务边界 → 整机 → 自造监督 → 整段任务 → flow 速度 → 块流式 → self-forcing DMD → Feature-RoPE cache → 训练 → 实验 → 局限”重排；不照论文三阶段标题直接平铺。
- 同一页拆清四个易混口径：论文量化最长 1 分钟、项目页宣称 hour-scale、13.6 FPS 能流式吐块但追不上 25 FPS 播放、官方仓库仍是 <code>Code coming soon</code>。
- 新增 swap-and-reconstruct-supervision、decoupled-streaming-conditioning-mask、efficient-multi-lora-switching、feature-rope-decomposition 四个概念页，并给 flow matching、DMD、KV cache、LoRA、RoPE 等既有概念补反向来源。
- 覆盖补充材料 §6–11：Feature-RoPE 定性消融、六步缓存事务、30 人盲测、1 分钟更多样例、单人/遮挡/表情局限与冒充风险；critic 完整目标和代码级 tensor 细节明确记为未公开。

### Review 1 · 全文、附录、证据与认知链

分数顺序：前置引入 / 中文表达 / 术语准确 / 比喻直觉 / 公式讲解 / 行文逻辑 / 配图。

| 节 | 7 维评分 | 本轮证据与发现 |
|---|---|---|
| §00 边界 | 5/5/5/4/4/5/4 | 把 1 分钟、hour-scale、13.6 vs 25 FPS、代码未公开拆成三卡一警示；无复杂公式。 |
| §01 总览 | 5/5/5/5/4/5/5 | 四路条件和三个训练阶段先拼完整闭环；Fig 01 每根箭头对应真实数据流。 |
| §02 数据 | 5/5/5/5/4/5/5 | 原片始终是标签、Seed-VC 只改条件写明；三条流水线分别落到 ViTPose/SAM2、变声与参考抽样。 |
| §03 Stage 1 | 4/5/5/4/5/5/4 | 双流拼接式逐下标解释，补共同物理时间；位置 offset 的作用已有动机但不另造论文未给的数。 |
| §04 Flow | 5/5/5/5/5/5/4 | 从直线路径求导出速度，所有符号就地定义；2/−1/.25 完整算到损失 .25。 |
| §05 Stage 2 | 5/5/5/5/5/5/5 | 3 latent→24 帧→.96 秒逐步换算；可见矩阵、teacher forcing loss 与 exposure gap 顺序完整。 |
| §06 Stage 3 | 5/5/5/5/5/5/5 | 三角色初始化/去留齐，CFG 数字例 2.6、critic 差 .5；明确 critic 完整 loss 未披露。 |
| §07 Feature-RoPE | 4/5/5/4/**3**/5/5 | 已有局部坐标和 100→103 滚窗图，但缺“未旋转 Key 怎样贴回相位”的显式公式；补成 <code>K_cache=k, K_attn=R(p̃)k</code> 与四格符号表。 |
| §08 训练 | 4/5/5/4/4/5/4 | 数据桶、精度、GPU、LoRA rank、三阶段步数/学习率与 critic 5:1 齐；无未公开规模外推。 |
| §09 实验 | 4/5/**3**/4/4/5/5 | 主表、长片、速度、消融和用户研究齐，但 Sync/DINO/IQA/SIG 首次出现没有直达解释；补四张指标翻译卡。 |
| §10 局限 | 5/5/5/4/4/5/4 | 任务、复现、证据、安全四类边界分开；小时级与 1 分钟证据不偷换。 |

第一轮修正后复算：<code>24/25=.96 s</code>、<code>24/1.76=13.636 FPS</code>、flow loss <code>.25</code>、CFG teacher <code>2.6</code>、DMD 差 <code>.5</code>、单层三 LoRA 教学账减少 <code>60.42%</code>，均与页面一致。

### Review 2 · 最终渲染页独立复核

| 节 | 7 维评分 | 最终页证据 |
|---|---|---|
| §00 边界 | 5/5/5/4/4/5/5 | 桌面三卡等高，手机单列；警示框清楚写出“流式 ≠ 实时”。 |
| §01 总览 | 5/5/5/5/4/5/5 | 桌面三段横排、手机箭头转向；去掉进入视口前的低透明动画，文字始终高对比。 |
| §02 数据 | 5/5/5/5/4/5/5 | 三路箭头在桌面居中、手机竖直；公式只渲染一次，不再出现 <code>A_t</code> 重复。 |
| §03 Stage 1 | 4/5/5/4/5/5/4 | 两条序列公式与 4 个符号格全部 KaTeX；手机无横向溢出。 |
| §04 Flow | 5/5/5/5/5/5/4 | 三个块公式和 4 步手算均实际渲染；没有裸反斜杠或跳步。 |
| §05 Stage 2 | 5/5/5/5/5/5/5 | 掩码表桌面完整、手机在局部容器横滑而页面宽度保持 390；颜色与 ✓/× 双编码。 |
| §06 Stage 3 | 5/5/5/5/5/5/5 | Teacher→Generator↔Critic 箭头端点居中；DMD 公式、噪声区间和符号格均在使用前后相邻。 |
| §07 Feature-RoPE | 5/5/5/5/5/5/5 | 新增缓存/使用 Key 公式；100/101/102→101/102/103 箭头不压字，六步 cache 事务顺序完整。 |
| §08 训练 | 4/5/5/4/4/5/4 | 四张配方卡在 1440 横排、390 单列；β₁/β₂ 已 KaTeX 化。 |
| §09 实验 | 5/5/5/5/4/5/5 | 指标解释先于结果表；宽表只在自身容器滚动，身份/画质/声音/速度结论没有混写。 |
| §10 局限 | 5/5/5/4/4/5/4 | 四类边界保持并列，来源链接可达，未把项目页宣传升级成论文结论。 |

最终浏览器验收：1440×1000 与 390×844 的页面宽度分别严格为 1440 / 390；8 个块公式全部生成 <code>katex-display</code>，58 个行内数学格全部 mathified，0 个 KaTeX error、0 个嵌套重复渲染；10 个术语入口与 10 个 glossary 条目一一对应，手机浮卡固定在底部而非屏幕中间；5 张图、13 个显式箭头无重叠；全新标签页控制台 0 error / 0 warning。

## [2026-08-19] ingest | Wave Forcing · 砍一条边，换一条流水线
- 无论文（信源=项目页+开源 runtime），按"博客版"口径入库并全页标注。海蓝 accent 小页 7 节：三代取舍 → 环变 DAG → 波前 → 藏通信 → VAE 实测切分 → 数字与质量（RF 硬套块因果推理 Drift 恶化 4.6× 的对照是全页题眼）。
- 首次引入 anime.js（CDN+SRI，17KB）做四个动画：✂ 砍反向边（环→DAG 显形）、GPU×tick 波前网格（播放/单步/重置+里程碑字幕，稳态 8 卡全满每 tick 12 帧）、同步 vs 重叠甘特（红色通信墙 vs 绿色传输沉入计算窗）、VAE FLOPs→实测时间切分 morph（146ms 瓶颈→95/99/96 齐步）。规则：**"东西随时间流动"的图走 anime.js 时间线，静态示意继续内嵌 SVG**。

### Review 1 · 项目页、README 与 commit b320dbc 逐项回查

| 节 | 7 维评分 | 修正与证据 |
|---|---|---|
| §00 版本 | 5/5/5/5/4/5/5 | 分开公开 1.3B 5-step Preview、未公开 4-step 最佳行与随机权重 14B 系统行，避免把吞吐当画质。 |
| §01–03 机制 | 5/5/5/5/5/5/5 | 先给端到端总览，再讲环变 DAG；明确块内全注意力、块间净→噪单向依赖以及旧权重硬切 mask 的失败对照。 |
| §04 runtime | 5/5/5/5/5/5/5 | 代码确认 `c=t-r`、`blocks+ranks-1`、确定性重加噪；纠正“VAE rank 也有完整 DiT”和“全系统零 collective”两处过度表述。 |
| §05–06 通信/VAE | 5/5/5/5/5/5/5 | 把 FP8、单边写、paged 拆成三种税；公式先亮、逐符号解释，再用 22/59/28→22/45/42 手算 min–max 切分。 |
| §07 实验 | 5/5/5/5/5/5/5 | 125.7 steady 与 117.7 E2E 均展示原公式和代入；50-prompt 质量表与 10-prompt DiT-only 表明确不可横比。 |
| §08 边界 | 5/5/5/5/4/5/5 | clean page 上限、工作窗截断、tick `all_gather`、anchor broadcast 与未公开训练细节均据代码标明。 |

### Review 2 · 最终 HTML 独立渲染复核

| 项目 | 结果 |
|---|---|
| 公式 | 4 个块公式与全部行内公式均由 KaTeX 渲染，0 error；`<` 已转义，不再吞掉后续正文。 |
| 交互 | 砍反向边、波前单步、同步/重叠甘特和 VAE 切分四组状态都实际点击；终态字幕与图一致。 |
| 桌面 | 1440×1000 下页面宽度严格 1440；五张图、结果表与总览没有箭头压字或卡片越界。 |
| 手机 | 390×844 下正文无横向溢出；宽表和长公式只在自身容器滚动，术语浮卡固定在屏幕底部且保持高对比。 |
| 静态门禁 | `lint_paper.py --warn waveforcing` 为 0 ERROR / 0 WARN，五个 `fig-*` id 唯一，`git diff --check` 通过。 |

## [2026-08-19] ingest | Avatar-Forever · 把“跑得快”和“跑得久”拆开学

- 不按论文目录平铺，改成“为什么是两门课 → 两支路怎样训练 → RRT 怎样造坏历史 → 权重怎样合并 → ForeverCache 怎样省计算 → 数据/配方/实验/边界”的学习路径。
- 新建四个直达概念页：<code>recovery-oriented-rollout-training</code>、<code>forever-cache</code>、<code>decoupled-parallel-adaptation</code>、<code>stop-gradient</code>；DMD、flow matching、KV cache 来源表同步挂上本论文。

### Review 1 · 论文主文与附录逐项回查

| 节 | 7 维评分 | 证据 |
|---|---|---|
| §00–02 拼图 | 5/5/5/5/4/5/5 | 先区分 22B 基座、DMD 全参、RRT LoRA、缓存四层；总览明确两支路都从 θ₀ 起步，不误写成先后串行。 |
| §03 DMD | 5/5/5/5/5/5/5 | 公式 (1) 完整列出、四类符号逐项定义；明确论文没有重新交代 real/fake score 网络训练，链接 DMD 前置页。 |
| §04 RRT | 5/5/5/5/5/5/5 | 公式 (2)–(5) 按执行顺序展开；K=4 六格时间线、stop-gradient 真实作用、σ=.25 的完整 flow 手算与 masked target 范围齐全。 |
| §05 合并 | 5/5/5/5/4/5/5 | 公式 (6) 与 DMD-only/FM-only/组合三角消融并排；明确参数增量可加不是普适定理，论文未做系数与冲突分析。 |
| §06 Cache | 5/5/5/5/5/5/5 | 公式 (7)–(9) 折成历史窗、populate/reuse、逐层缓存三步；12→6 教学账后紧跟 23.6%/45.5% 实测，未偷换成 2×。 |
| §07–08 数据/配方 | 5/5/5/5/4/5/5 | MDD→GPT→LTX→语义/视觉/运动筛选齐；22B、rank/alpha 128、K=4、30-step、退化概率、lr、batch、训练步数全部覆盖。 |
| §09–10 实验/边界 | 5/5/5/5/5/5/5 | 5 秒、30 秒和 11 分钟证据分账；延迟降幅与吞吐增幅分别复算；代码、权重、数据规模、消费硬件、无限证据边界齐。 |

第一轮复算：\(1.25=.75\times2+.25\times(-1)\)、\(v^*=-1-2=-3\)、30 秒延迟下降 \(31.2\%\)、吞吐提升 \(45.5\%\)、DMD+RRT 相对 DMD-only Overall \(+3.6\%\)、FVD \(-16.5\%\)，均与页面和原文一致。

### Review 2 · 最终网页独立复核

| 项目 | 结果 |
|---|---|
| 公式 | 10 个块公式全部生成 <code>katex-display</code>，行内公式全部 mathified，0 KaTeX error；所有参数都在首次使用前后定义。 |
| 图与箭头 | 6 张 <code>fig-*</code> id 唯一；箭头全部作为 grid 独立单元或卡片间连接，不使用容易偏移的绝对定位；手机端横向箭头自动转为纵向。 |
| 交互 | RRT 的 +2→+3→+5→+7 传播和 Cache 的 12→6 成本账都实际点击，按钮终态、字幕和格子一致。 |
| 桌面 | 1440×1000 页面宽度严格 1440；总览、时间线、表格、公式没有重叠或低对比文本。 |
| 手机 | 390×844 页面宽度严格 390；宽 cache 表只在自身容器滚动；术语浮卡固定底部，文字/背景高对比。 |
| 静态门禁 | <code>lint_paper.py --warn avatar-forever</code> 为 0 ERROR / 0 WARN，缺失概念引用为 0，<code>git diff --check</code> 通过。 |
