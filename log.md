...
## [2026-07-16] expand | krea-2 数据过滤术语补课 — 补清伪影与有意视觉风格的边界、高频/结构伪影实例；把常见 pHash 拆成缩放灰度→DCT→保留低频→中位数二值化→汉明距离，解释 8×8 版先缩 32×32、保留 64 位，12×12 版先缩 48×48、保留 144 位，并说明 colorhash 看颜色分布但不看位置、Krea 未披露判重阈值与合并规则；用 3×3 Laplacian 核手算纯色响应 0、孤立亮点 −600，标明报告未公开具体核与阈值；术语表 13→16
